...
public class AmbassadorProfileController : Controller
{
    ...
    public async Task<IActionResult> Index(CancellationToken cancellationToken)
    {
        ...
        var apprenticeshipDetails = memberProfiles.Result.Apprenticeship != null ? new ApprenticeshipDetailsModel(memberProfiles.Result.Apprenticeship?.Sector, memberProfiles.Result.Apprenticeship?.Programmes, memberProfiles.Result.Apprenticeship?.Level) : null;

        AmbassadorProfileViewModel model = new(personalDetails, memberProfiles.Result.Email, memberProfiles.Result.Profiles, memberProfiles.Result.Preferences, apprenticeshipDetails, profiles.Result.Profiles);
        return View(model);
    }
}